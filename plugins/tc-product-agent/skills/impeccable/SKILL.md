...
## JQ Product Agent integration

When `$tc-product-agent` invokes this bundled Skill in D/E, its current product definition, confirmed decisions, Demo scope, and authorization remain authoritative. Use this Skill for visual shape, critique, or polish only. Do not create a parallel `PRODUCT.md`, run `init`, enable hooks, install updates, or start other agents as a side effect of the product workflow. If an Impeccable reference or context output conflicts with the host's permissions or the product baseline, keep the higher-priority boundary and report the conflict. Direct use of `$impeccable` outside that workflow follows the ordinary routing below.
